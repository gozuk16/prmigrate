...
`config.toml` で Bitbucket nickname / UUID / Atlassian account ID → GitHub username を定義。
...
prmigrate migrate --config config.toml --repo myteam/myrepo --dry-run

# 3. 本実行
prmigrate migrate --config config.toml --repo myteam/myrepo
```
